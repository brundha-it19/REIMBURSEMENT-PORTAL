<template>
    <div>       
     ...
     <i class="fa fa-user" id="user" aria-hidden="true" style="font-size:20px;color:black;text-align: center;">&nbsp;<button class="btn" @click="logout()">Logout</button> </i>
       ...
       <p style="font-family:cursive;"><b> Financier login</b></p>
       <div class="topnav">           
...
<div class="sidenav">
        <br/>
        <br/>
  <a href="http://localhost:8080/finance" style="font-family:cursive;margin-right:20px;font-size:20px;color:white;"> Claims are waiting for your approval in this page..!</a>
  <br/>
  <br/>
  </div>
  </header>
  ...
  <tbody>
    <tr>
      <td>
        {{ users.data.data[0].claimId }}
      </td>
      <td>
        {{ users.data.data[0].category }}
      </td>
      <td>
        {{ users.data.data[0].description }}
      </td>
      <td>
        {{ users.data.data[0].amount}}
      </td>
     <td> {{ users.data.data[0].statusOfApprovers[0].status}}</td>
     <td> {{ users.data.data[0].statusOfApprovers[1].status}} &nbsp; <button
          class="btn btn-primary" id="coll1" data-toggle="collapse" data-target="#collapseExample2" aria-expanded="false" aria-controls="collapseExample"  @click="claim_details()">expand</button></td>
    </tr>
    <div id="collapseExample2" class="collapse" aria-labelledby="headingOne" data-parent="#accordion">
      <div class="card card-body"  id="coll2">
        ...
        <input type="text" id="inp" v-model="comments"/>
    ...
    <tr>
      <td>
        {{ users.data.data[1].claimId }}
      </td>
      <td>
        {{ users.data.data[1].category }}
      </td>
      <td>     
        {{ users.data.data[1].description }}
      </td>
      <td>
        {{ users.data.data[1].amount}}
      </td>
    ...
    <div id="collapseExample3" class="collapse" aria-labelledby="headingOne" data-parent="#accordion">
      ...
      <!-- <tr>
      <td>
        {{ users.data.data[2].claimId }}
      </td>
      <td>
        {{ users.data.data[2].category }}
      </td>
      <td>     
        {{ users.data.data[2].description }}
      </td>
      <td>
        {{ users.data.data[2].amount}}
      </td>
     <td> {{ users.data.data[2].statusOfApprovers[0].status}}</td>
     <td> {{ users.data.data[2].statusOfApprovers[1].status}} &nbsp; <button
          class="btn btn-primary" id="coll2" data-toggle="collapse" data-target="#collapseExample3" aria-expanded="false" aria-controls="collapseExample">expand</button></td>
    </tr> -->

    <!-- <div id="collapseExample3" class="collapse" aria-labelledby="headingOne" data-parent="#accordion">
      <div class="card card-body"  id="coll2">
        Claim I'd : {{ users.data.data[2].claimId }}<br/>
        Employee name: {{ users.data.data[2].employeeName }}<br/>
        Claim category :{{ users.data.data[2].category  }}<br/>
        Claim Description :{{ users.data.data[2].description }}<br/>
        Claim Amount :{{ users.data.data[2].amount }}<br/>
        From Date : {{ users.data.data[2].statusOfApprovers[0].status }}<br/>
        To Date :{{ users.data.data[2].statusOfApprovers[1].status }}<br/>
  ...
  <script src="/Users/brundha/Documents/vue/project/my-project/src/components/finance.js">    </script>
  <style scoped>
  @import '/Users/brundha/Documents/vue/project/my-project/src/components/finance.css'
  </style>